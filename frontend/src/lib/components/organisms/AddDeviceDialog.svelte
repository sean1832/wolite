...
<Dialog.Root bind:open>
	<Dialog.Trigger>
		{#snippet child({ props })}
			...
			{:else}
				<Button {...props} size="sm" class="h-9 gap-2 border border-border/50 px-4 shadow-sm">
					<Plus class="h-4 w-4" />
	...
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			...
			<Dialog.Description>Add a new computer to wake up remotely.</Dialog.Description>
		</Dialog.Header>
